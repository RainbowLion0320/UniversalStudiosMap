...
export class ParkMap {
 constructor(container,onSelect){
  this.container=container;this.onSelect=onSelect;this.pins=[];this.labels=[];this.pendingFrame=null;
  ...
  new GLTFLoader().load('/models/beijing-park.glb',gltf=>{
   this.model=gltf.scene;gltf.scene.traverse(obj=>{if(obj.isMesh){obj.castShadow=true;obj.receiveShadow=true;}});this.scene.add(gltf.scene);this.loading.remove();this.requestRender();
 ...
 requestRender(){
  if(this.pendingFrame!==null||document.hidden||!this.renderer)return;
  this.pendingFrame=requestAnimationFrame(()=>{this.pendingFrame=null;if(document.hidden)return;this.renderer.render(this.scene,this.camera);this.projectLabels();});
 }
 fitBounds(){this.controls.target.set(0,0,-2);this.camera.position.set(74,115,125);this.camera.zoom=this.container.clientWidth/this.container.clientHeight>1.4?1.25:1;this.controls.update();this.camera.updateProjectionMatrix();this.requestRender();}
 topView(){this.camera.position.set(0,160,1);this.controls.target.set(0,0,-2);this.controls.update();this.requestRender();}
 zoom(delta){this.camera.zoom=THREE.MathUtils.clamp(this.camera.zoom*delta,.55,4.5);this.camera.updateProjectionMatrix();this.requestRender();}
 flyTo(latlng){const p=position(latlng),offset=this.camera.position.clone().sub(this.controls.target);this.controls.target.copy(p);this.camera.position.copy(p).add(offset);this.camera.zoom=Math.max(this.camera.zoom,1.65);this.controls.update();this.camera.updateProjectionMatrix();this.requestRender();}
 invalidateSize(){const {width,height}=this.container.getBoundingClientRect();if(!width||!height)return;const aspect=width/height,span=aspect<.8?122/aspect:118;this.camera.left=-span*aspect/2;this.camera.right=span*aspect/2;this.camera.top=span/2;this.camera.bottom=-span/2;this.camera.updateProjectionMatrix();this.renderer.setSize(width,height);this.requestRender();}
 ...
 projectLabels(){
  ...
  const occupied=[];
  for(const entry of [...this.pins].sort((a,b)=>Number(b.focused)-Number(a.focused)||Number(b.planned)-Number(a.planned)).concat(this.labels)){const p=entry.point.clone().project(this.camera),x=(p.x*.5+.5)*width,y=(-p.y*.5+.5)*height;entry.el.style.transform=`translate(${x}px, ${y}px) translate(-50%, -100%)`;const hidden=p.z>1||p.z< -1||x<0||x>width||y<0||y>height;const collision=!entry.planned&&!entry.focused&&occupied.some(q=>Math.hypot(q.x-x,q.y-y)<36);entry.el.style.display=hidden||collision?'none':'';if(!hidden&&!collision)occupied.push({x,y});entry.el.style.zIndex=entry.planned?1000:Math.round((1-p.z)*100);}
 }
}
