import { loadPark,refreshPark,sharePlan,registerBack,native } from './data-service.js';
...
function focusItem(id){active=id;const p=items.find(x=>x.id===id);if(!p)return;map.flyTo(p.latlng,18,{duration:.45});renderDetail(p);renderMarkers();setMobile('map');}
function setMobile(view){mobile=view;document.body.dataset.view=view;document.querySelectorAll('[data-view-tab]').forEach(el=>{el.classList.toggle('active',el.dataset.viewTab===view);el.setAttribute('aria-pressed',String(el.dataset.viewTab===view));});setTimeout(()=>map?.invalidateSize(),30);}
function shell(){
 $('#app').innerHTML=`
 <header class="topbar"><button class="brand" data-view-tab="map" aria-label="返回地图"><span class="brand-mark">${icon('route')}</span><span>环球漫游<small>北京环球影城</small></span></button><div class="header-actions"><span id="save-status">本地行程</span><button class="icon-button" id="menu" aria-label="更多选项">${icon('info')}</button></div></header>
 <main class="workspace">
 <aside class="explore panel"><div class="panel-heading"><div><h2>想玩什么？</h2><p>点一下 +，加入这一天。</p></div><button class="icon-button" data-view-tab="map" aria-label="收起项目清单">${icon('x')}</button></div>
 ...
 <div class="list-heading"><span id="results-count"></span></div><div id="cards" class="cards"></div>

 </aside>
 <section class="map-panel" aria-label="园区互动地图"><div id="map"></div><div class="map-top"><span class="map-tag"><span class="dot"></span>微缩园区</span><div class="map-top-actions"><button id="fit" class="map-button" title="显示整个园区" aria-label="显示整个园区">${icon('locate')}</button></div></div><div class="view-controls"><button id="top-view" title="俯视地图" aria-label="俯视地图">俯视</button><button id="zoom-in" aria-label="放大地图">+</button><button id="zoom-out" aria-label="缩小地图">−</button></div><div class="north"><span>N</span><svg viewBox="0 0 28 35" aria-hidden="true"><path d="m14 3 10 27-10-6-10 6Z" fill="#315b51"/><path d="m14 3 0 21 10 6Z" fill="#8caa9b"/></svg></div>
 <div id="detail" class="detail hidden"></div><div class="map-credit">© <a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noreferrer">OpenStreetMap</a> contributors</div><div class="map-gesture">拖动旋转 · 双指缩放与平移</div><div class="map-bottom"><div class="map-legend"><span><i class="line"></i>步道估算</span><span><i class="line dashed"></i>入口待核对</span></div></div></section>
 <aside class="itinerary panel"><div class="panel-heading"><div><h2>我的一天 <span id="plan-count"></span></h2><p>自动保存，随时继续。</p></div><button class="icon-button" data-view-tab="map" aria-label="收起行程">${icon('x')}</button></div>
 <button class="trip-date" id="trip-date" aria-label="修改游玩日期和出发时间"></button>
 <div id="summary"></div><div class="plan-actions"><button id="optimize" class="subtle">${icon('sparkles')}减少折返</button><button id="edit-mode" class="subtle">调整行程</button></div>
 <div id="timeline" class="timeline"></div><div class="plan-footer"><div id="warnings"></div><div class="source-line"><span id="source-label"></span></div><a href="https://themeparks.wiki/" target="_blank" rel="noreferrer">Powered by ThemeParks.wiki</a></div></aside>
 </main><nav class="mobile-nav" aria-label="工作区切换"><button data-view-tab="explore">${icon('search')}选项目</button><button class="active" data-view-tab="map">${icon('map')}看地图</button><button data-view-tab="plan">${icon('route')}行程 <b id="mobile-count"></b></button></nav>
...
function renderPlan(){
 $('#trip-date').innerHTML=`${icon('sun')}<span>${e(settings.date)}<b>${e(settings.start)} 出发</b></span><span class="trip-edit">修改</span>`;
 $('#plan-count').textContent=`${selected.length} 站`;$('#mobile-count').textContent=selected.length;
 $('#summary').innerHTML=`<div class="summary-card"><div class="summary-top"><span>${plan.unknownLegs?'已知部分预计':'这一天预计'}</span><strong>${Math.floor(plan.total/60)}<small>小时</small>${plan.total%60}<small>分钟</small></strong></div><p class="estimate-mode">${plan.useLive?'当天排队快照 · 手动调整优先':'排队按默认估算 · 可逐站调整'}</p><div class="summary-metrics"><span>${icon('footprints')}步行 <b>${plan.walk}′${plan.unknownLegs?'+':''}</b></span><span>${icon('clock')}排队 <b>${plan.queue}′</b></span><span>${icon('sun')}休息 <b>${plan.rest}′</b></span></div></div>`;
 $('#timeline').classList.toggle('editing',editing);
 $('#timeline').innerHTML=plan.rows.length?``+plan.rows.map((r,index)=>`<article class="stop ${done.has(r.item.id)?'completed':''}" draggable="true" data-stop="${r.item.id}">
 ...
 <div class="stop-tools"><button data-move="${r.item.id}" data-direction="-1" aria-label="上移${e(r.item.zh)}" ${index===0?'disabled':''}>${icon('arrow-up')}</button><button data-move="${r.item.id}" data-direction="1" aria-label="下移${e(r.item.zh)}" ${index===plan.rows.length-1?'disabled':''}>${icon('arrow-down')}</button><button class="edit-stop" data-edit="${r.item.id}">调整时间</button><button class="remove-stop" data-add="${r.item.id}" aria-label="从行程移除${e(r.item.zh)}">${icon('x')}</button></div></div><span class="drag-grip" title="拖动调整顺序">${icon('grip')}</span></div></article>`).join('')+`<div class="finish"><span>${icon('flag')}</span><div><strong>${timeLabel(plan.finish)}${plan.unknownLegs?' 之后':''} · 预计结束</strong><small>含 ${plan.rest} 分钟自由休息，安排在行程末尾</small></div></div>`:`<div class="empty"><span>${icon('route')}</span><h3>把心动，加入这一天</h3><p>从地图或清单选择项目，<br>你的路线会在这里慢慢展开。</p><button class="button primary" id="starter">试试经典五站</button></div>`;
 $('#warnings').innerHTML=plan.warnings.map(w=>`<p class="warning">${icon('info')}${e(w)}</p>`).join('');
 $('#source-label').textContent=sourceTime()?`数据快照 ${fmtDate(sourceTime())}`:'离线模式 · 排队按估算';
...
function renderDetail(p){
 ...
 $('#detail').innerHTML=`<button class="detail-close" id="close-detail" aria-label="关闭项目详情">${icon('x')}</button><small style="color:${ZONES[p.zone].color}">${e(ZONES[p.zone].name)}</small><h3>${e(p.zh)}</h3><div class="detail-stats"><span>${icon('clock')}体验约 ${p.duration} 分钟</span><span>${Number.isFinite(wait)?`快照排队 ${wait} 分钟`:'暂无排队数据'}</span></div><p class="detail-note">${sourceTime()?`${fresh?'':'历史快照 · '}${fmtDate(sourceTime())}`:'离线估算'} · 入口请核对现场</p><button class="button ${selected.includes(p.id)?'outline':'primary'}" data-detail-add="${p.id}">${icon(selected.includes(p.id)?'check':'plus')}${selected.includes(p.id)?'已加入 · 点击移除':'加入我的一天'}</button>`;
}
function openDialog(content){$('#dialog-content').innerHTML=content;$('#dialog').showModal();}
function dateDialog(){
 openDialog(`<h2>哪天去玩？</h2><form id="date-form" class="settings-form"><label>游玩日期<input id="date" name="date" type="date" value="${settings.date}" required></label><label>第一站抵达时间<input id="start" name="start" type="time" value="${settings.start}" required></label><button class="button primary" type="submit">保存出发时间</button></form>`);
 $('#date-form').onsubmit=event=>{event.preventDefault();const data=new FormData(event.currentTarget);if(settings.date!==data.get('date'))for(const o of Object.values(settings.overrides))delete o.showTime;settings.date=data.get('date');settings.start=data.get('start');$('#dialog').close();refreshPlan();};
}
function settingsDialog(){
 openDialog(`<h2>游玩节奏</h2><p class="dialog-lead">修改估算值，路线与时间轴会一起更新。</p><form id="settings-form" class="settings-form"><label>步行速度<select name="speed"><option value="2.5">慢慢逛 · 2.5 km/h</option><option value="3.5">轻松走 · 3.5 km/h</option><option value="4.5">快步走 · 4.5 km/h</option></select></label><label>默认排队时间（分钟）<input name="defaultQueue" type="number" min="0" max="180" value="${settings.defaultQueue}" required></label><label>预留用餐 / 休息（分钟）<input name="rest" type="number" min="0" max="240" value="${settings.rest}" required></label><label class="checkbox"><input name="avoidSteps" type="checkbox" ${settings.avoidSteps?'checked':''}>寻路时避开已标记的台阶</label><label class="checkbox"><input name="useLive" type="checkbox" ${settings.useLive?'checked':''}>当天规划时，使用刚刷新的排队与场次</label><p class="form-note">仅在游玩日期为今天、数据抓取不超过 15 分钟时启用。未来日期或旧快照使用默认估算；手动调整优先。避开台阶不代表已验证无障碍可达。</p><button class="button primary" type="submit">更新我的行程</button></form>`);
...
function editStop(id){
 ...
 openDialog(`<h2>${e(row.item.zh)}</h2><form class="settings-form" id="stop-form"><label>排队时间（分钟）<input name="wait" type="number" min="0" max="240" value="${row.wait}" required></label><label>体验 / 演出时长（分钟）<input name="duration" type="number" min="1" max="240" value="${row.duration}" required></label>${row.item.entityType==='SHOW'?`<label>固定演出场次（可留空）<input name="showTime" type="time" value="${e(override.showTime??'')}"></label><p class="form-note">手动场次仅适用于当前游玩日期；建议至少提前 10 分钟抵达。</p>`:''}<div class="dialog-actions"><button type="button" id="reset-stop" class="button outline">恢复估算</button><button class="button primary" type="submit">保存调整</button></div></form>`);
...
async function exportPlan(){
 ...
 await sharePlan(text,settings.date);
}
async function updateData(notify=true){
 try{raw=await refreshPark(raw);items=enrich(raw.children,raw.live);refreshPlan();if(active)renderDetail(items.find(x=>x.id===active));if(notify)toast('排队与演出信息已更新');}catch{if(notify)toast('暂时连不上数据源，离线地图仍可使用');}
}
function menuDialog(){
 openDialog(`<h2>环球漫游</h2><p class="dialog-lead">你的行程只保存在这台设备上。</p><div class="menu-options"><button id="settings">${icon('clock')}游玩节奏<span>排队、休息与步速</span></button><button id="export">${icon('download')}${native?'分享行程':'导出行程'}</button><button id="refresh">${icon('refresh')}更新排队与演出<span>${sourceTime()?fmtDate(sourceTime()):'联网后可更新'}</span></button><button id="about">${icon('map')}地图与数据说明</button></div>`);
 $('#settings').onclick=settingsDialog;$('#export').onclick=()=>{$('#dialog').close();exportPlan();};$('#about').onclick=aboutDialog;$('#refresh').onclick=()=>{$('#dialog').close();updateData();};
}
function bind(){
 ...
 $('#fit').onclick=fitMap;$('#top-view').onclick=()=>map.topView();$('#zoom-in').onclick=()=>map.zoom(1.25);$('#zoom-out').onclick=()=>map.zoom(.8);
 $('#menu').onclick=menuDialog;
 $('#edit-mode').onclick=()=>{editing=!editing;$('#edit-mode').textContent=editing?'完成调整':'调整行程';renderPlan();};
 ...
 document.addEventListener('click',event=>{
  ...
  if(button.id==='close-detail'){$('#detail').classList.add('hidden');active=null;renderMarkers();}
 ...
 document.addEventListener('keydown',event=>{if(event.key==='/'&&!['INPUT','TEXTAREA','SELECT'].includes(event.target.tagName)&&!$('#dialog').open){event.preventDefault();setMobile('explore');$('#search').focus();}});
}
async function start(){
 try{
  ...
  registerBack(()=>{if($('#dialog').open){$('#dialog').close();return true;}if(active){$('#detail').classList.add('hidden');active=null;renderMarkers();return true;}if(mobile!=='map'){setMobile('map');return true;}return false;});
  if(!liveUsable(shanghaiDate(),sourceTime()))updateData(false);
  window.addEventListener('online',()=>updateData(false));
